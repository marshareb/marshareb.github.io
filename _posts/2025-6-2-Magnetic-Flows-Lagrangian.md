...
To start, given two points $p,q \in N$, let $C_{p,q}$ be the collection of $C^2$-curves of the form $\gamma :[0,1] \rightarrow N$, where $\gamma(0) = p$ and $\gamma(1) = q$. Recall that an *endpoint-fixed variation* of a curve $\gamma \in C_{p,q}$ is a smooth family of curves $(\gamma_s) \subseteq C_{p,q}$ such that $\gamma_0 = \gamma$. Differentiating $\gamma_s$ at $s=0$ yields a vector field $V$ along $\gamma$. The *tangent space* for $C_{p,q}$ consists of all of these variational vector fields $V$; we write $T_\gamma C_{p,q}$ for this collection. Under the appropriate definitions, this turns out to actually be the tangent space in a differential geometry sense, which we'll just accept for the time being.
