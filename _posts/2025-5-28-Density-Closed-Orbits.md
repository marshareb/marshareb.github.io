...
It is well-known that $$\Gamma$$ is dense in $$X_A^r$$. One can see this using transitivity of the flow along with the Anosov closing lemma, however, let's show this to be true using equidistribution. If $$C \subseteq X_A^r$$ is an open set, then let's write $$\chi_C$$ as a function on $$P$$, where $$\chi_C(\gamma)$$ is the integral of $$\chi_C$$ over the curve $$\gamma$$. If $$\mu$$ is the measure of maximal entropy, then one way of interpreting equidistribution is that we have
