...
The *local stable/unstable manifold theorem* says there is a uniform $\epsilon > 0$ such that the sets $W^{s}_{\epsilon}(x)$ and $W^{u}_{\epsilon}(x)$ are actually $C^{\infty}$-embedded disks for all $x\in M$ (and hence the "manifold" part of the name is not arbitrary). Also important is the following: There is a uniform $\eta > 0$ such that if $d(x,y) < \eta$, then 
