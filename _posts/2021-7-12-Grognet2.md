...
Let $$\delta(t) := \gamma(-t)$$ be the reverse flow. Both these cut out the same curve, so we have

$$
\begin{split}
\dot{\gamma}(t) i \kappa(\gamma(t)) = \frac{D}{dt} \dot{\gamma}(t) \\
= - \frac{D}{dt} \dot{\delta}(t) = - \kappa(\delta(t)) i \dot{\delta}(t).
\end{split}
$$

Using the fact that $$\dot{\delta}(t) = - \dot{\gamma}(-t),$$ we get

$$ \dot{\gamma}(t) i \kappa(\gamma(t)) = \kappa(\gamma(-t)) i \dot{\gamma}(-t).$$

Using the fact that $$\dot{\gamma}(-t) = - \dot{\gamma}(t),$$ we have

$$ - \kappa(\gamma(t)) = \kappa(\gamma(-t)).$$

So $$\kappa \circ \gamma : \mathbb{R} \rightarrow \mathbb{R}$$ is an odd function. Simultaneously, we see that

$$
\begin{split}
-\frac{D}{dt} \dot{\gamma}(t) = \frac{D}{dt} \dot{\gamma}(-t) = i\kappa(\gamma(-t)) \dot{\gamma}(-t) \\
=   -i \kappa(\gamma(-t)) \dot{\gamma}(t),
\end{split}
$$

so we get

$$ i \kappa(\gamma(-t)) \dot{\gamma}(t) = i \kappa(\gamma(t)) \dot{\gamma}(t).$$

Hence $$\kappa \circ \gamma$$ is also even, which forces it to be $$0$$. Since $$\kappa$$ is $$0$$ on all periodic orbits and periodic orbits are dense, the result follows.
