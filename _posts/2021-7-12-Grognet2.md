...
**Definition:** We define a **cord** to be a geodesic curve connecting two points. Given a curve $$c$$, we say that $$c$$ **tends to infinity** if $$d(c(0),c(t)) \rightarrow \infty$$ as $$|t| \rightarrow \infty.$$
...
**Lemma 5.5:** If the uniform norm of the magnetic field is strictly less than $$k_1$$ then there exists a unique geodesic asymptote to the curve.
...
The idea is that $$\xi(v)$$ spits out the smallest angle $$\theta$$ between $$0$$ and $$\pi$$ so that the flow given by the vector $$\theta \cdot v$$ has the same origin and terminal point on the boundary at infinity. Analogously, $$\xi_1(v)$$ spits out the smallest angle $$\theta$$ between $$0$$ and $$\pi$$ so that the flow given by $$\theta \cdot v$$ has the same terminal point on the boundary at infinity. Finally $$\xi_2(v)$$ spits out the smallest angle $$\theta$$ between $$0$$ and $$\pi$$ so that the flow given by $$\theta \cdot v$$ has the same origin point on the boundary at infinity. Since everything is Anosov, taking a curve with prescribed curvature $$c(t)$$, we have that the maps
...
$$\frac{1}{T} \int_0^T (1- \cos(\xi(\gamma(t)))) dt = \frac{1}{T} \int_0^T \int_0^{\xi(\gamma(t))} \sin(\theta) d\theta dt = \frac{1}{l(\gamma')} \int_0^{l(\gamma')} \int_0^\pi \sin(\theta) d\theta dt = 2.$$

Thus we get that $$\xi(\gamma(t)) = \pi$$ for all $$t$$ in the interval $$[0,T]$$. Thus the only vector with angle between $$(0,\pi]$$ with the same origin and terminal point at the boundary at infinity is going to be the one that points in the opposite direction.

The trick now is as follows. We are given a unique geodesic which connects the two endpoints on the boundary at infinity and which stays at bounded distance from our flow. Pick a point $$\gamma(t)$$ along this closed orbit. We can let $$c_t(s)$$ be the orbit going in reverse direction. We now measure the distance between this and the geodesic flow $$\delta$$.
