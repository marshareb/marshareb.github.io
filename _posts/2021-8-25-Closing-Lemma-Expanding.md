...
We'll try to mimic this result for general expanding maps. The doubling map can be found in Problem 24 in my [dynamics problems](https://marshareb.github.io/files/Dynamics.pdf).
...
The other thing to note here is that we can actually get a variation of **Proposition 6.4.16** from Katok and Hasselblatt. Namely if $$x,y \in S^1$$ satisfy the property that $$d(f^k(x), f^k(y)) < \delta$$ for $$\delta$$ sufficiently small and $$0 \leq k \leq n$$
