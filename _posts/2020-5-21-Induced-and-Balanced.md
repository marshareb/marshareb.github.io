...
It's still mostly building terminology right now. A lot of the past week has been focusing on seminorms and the topology they generate. A seminorm is essentially just a norm without the positive definiteness property. Hence, a *seminorm* $$\rho : X \rightarrow \R_{\geq 0}$$ is a map which satisfies the triangle inequality, i.e. $$\rho(x+y) \leq \rho(x) + \rho(y)$$ for all $$x, y \in X$$, and it satisfies a homogeneity property, i.e. $$\rho(\alpha x) = |\alpha| \rho(x)$$ for all $$\alpha$$ scalars and $$x \in X$$. A family of seminorms $$\mathcal{F}$$ on a topological vector space $$X$$ is said to be *separating* if, for all $$x \noteq 0$$ in $$X$$, we have that there exists some $$\rho \in \mathcal{F}$$ so that $$\rho(x) \neq 0$$ as well. In some sense, the family of functions sees the difference between points, even if the individual functions don't necessarily see the difference.
